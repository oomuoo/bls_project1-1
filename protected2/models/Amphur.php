...
class Amphur extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('code, name, geo_id, province_id', 'required'),
			array('code, geo_id, province_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, code, name, geo_id, province_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'districts' => array(self::HAS_MANY, 'District', 'amphur_id'),
			'members' => array(self::HAS_MANY, 'Member', 'amphur_id'),
		);
	}
