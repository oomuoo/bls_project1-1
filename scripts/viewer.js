...
var PDFView = {
  ...
  getVisiblePages: function pdfViewGetVisiblePages() {
    ...
    if (tp_Range) {
		for (var i = tp_Range[0]; i <= tp_Range[1]; i++) {
		var singlePage = pages[i - 1];
		visiblePages.push({ id: singlePage.id, y: null,
							view: singlePage });
		}
	}
    return visiblePages;
...
var PageView = function pageView(container, pdfPage, id, scale,
  ...
  // div.className = 'page';
  div.className = 'page ' + ((this.id%2==0) ? 'odd' : 'even');
  // $('#viewer').turn('addPage', div, this.id);
...
window.addEventListener('scalechange', function scalechange(evt) {
  ...
  var page = PDFView.pages[0];
  var w = page.width;
  var h = page.height;
  
  $('#viewer').turn('size', w*2, h);
