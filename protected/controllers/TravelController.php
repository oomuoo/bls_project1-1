...
class TravelController extends Controller
{
	...
	public function actionTravel()
	{
		...
		$results = Yii::app()->db->createCommand()-> select('book.id, book.category_id,book.create_date, book.description, book.member_id, book.picture, book.name, member.alias')-> from('book')->leftJoin('member', 'book.member_id = member.id')-> where('book.category_id=4')-> order ('book.id DESC')-> queryAll();
		$this->render('travel',array(
				'travel'=>$results
		));
	}
